    ...
    Resolver<AssetType>
{
    ...
    fn new(path: assets::file::ConsPath, path_prefix: &str) -> ggez::GameResult<Self> {
        let fs = path.fs();
        let path_prefix = path_prefix.to_string();

        // Load the resolver file's bytes
        let bytes = match assets::file::try_bytes(path.into()) {
            Ok(bytes) => bytes,
            Err(e) => {
    ...
    fn try_new(path: assets::file::ConsPath, path_prefix: &str) -> Option<Self> {
    ...
    fn try_get(
        ...
        assets::file::try_bytes(assets::file::Path::new(self.fs, path))
    }
    ...
    pub fn new() -> ggez::GameResult<Self> {
        Ok(Self {
            internal_sprite: Resolver::<Sprite>::try_new(
                assets::file::ConsPath::new(
                    assets::file::FileSystem::Internal,
                    "sprite/resolver.ron",
                ),
                "sprite/",
            ),
